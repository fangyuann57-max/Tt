...
v3 architectural refactor (the 5 requested issues):
    ...
    5. MEMORY-EFFICIENT UPLOADING
       - send_video()/send_photo() receive the on-disk *path*
...
v3.5 zero-choice auto-download:
    - Removed the Video/Audio format keyboard entirely. Sending a link
      now downloads and sends it back immediately — video or photo,
      whichever the link actually is. Only a Cancel button remains.
    - Removed the "Audio only" / MP3-extraction feature and its
      FFmpegExtractAudio postprocessor — no longer reachable from the UI,
      so the dead code path was deleted rather than left unused.
...
# Extensions used to decide whether to send the file as a photo or a video.
...
def download_video(
    ...
    """Download a video or photo using yt-dlp. Runs in a worker thread.
...
def download_with_retry(
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        filepath, error = download_video(
            url, platform, out_dir, progress_state, cancel_event
        )
        if filepath:
            return filepath, None
...
# ----------------------------------------------------------------------------
# URL-based photo detection — used to pick the status-message icon, and by
# download_video()'s format cascade to prefer image formats for photo posts.
# ----------------------------------------------------------------------------
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # No command, no choices — every valid link auto-downloads immediately
    # and sends back whatever it actually is: video, or photo.
    ...
    await run_download_and_send(
        context, chat_id, user.id, url, platform, status_message=status_message,
    )
...
# strict cleanup. Sends whatever the link actually contains: video or photo.
# ----------------------------------------------------------------------------
async def run_download_and_send(
    ...
    try:
        try:
            filepath, error = await asyncio.wait_for(
                asyncio.to_thread(
                    download_with_retry,
                    url, platform, str(work_dir), progress_state, cancel_event,
                ),
    ...
    try:
        if error or not filepath or not os.path.exists(filepath):
            await record_download(user_id, platform, "video", 0, success=False)
        ...
        if size_mb > MAX_FILE_SIZE_MB:
            await record_download(user_id, platform, "video", size_bytes, success=False)
            await status_message.edit_text(
        ...
        try:
            ...
            pass
        await record_download(user_id, platform, media, size_bytes, success=True)
    except Exception as e:
