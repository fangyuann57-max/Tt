...
async def handle_link(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    status = await message.reply_text(
        f"⬇️ Getting ready to download… this can take a few minutes for "
        f"large videos (up to 15 min max)."
    )
    ...
    # A progress_updater task runs alongside it, editing `status` every few
    # seconds with a live progress bar sourced from yt-dlp's own hooks.
    os.makedirs(TEMP_DIR, exist_ok=True)
    progress_state: dict = {"status": "starting"}
    stop_flag = {"done": False}
    updater_task = asyncio.create_task(
        run_progress_updates(status, progress_state, stop_flag)
    )
    try:
        filepath, error = await asyncio.wait_for(
            asyncio.to_thread(download_video, url, platform, TEMP_DIR, progress_state),
